...
public class YelpAPI {
    ...
    private static final String CLIENT_ID  = "DEfCWiVeQ07wi40wWfIerQ";
    private static final String CLIENT_SECRET = "FGsH1J5BNApeRBhDzRXRjHri8NRnqM97uyAItagJgbb7qNs0yfxPGltI5f4i9WWf";
    private static final String GRANT_TYPE = "client_credentials";
    private static final String TOKEN_TYPE = "Bearer";


    //cache access token for faster access
    ...
    private List<Item> getItemList(JSONArray array) throws JSONException {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            builder.setCategories(categories).setName(object.getString("name")).setImageUrl(object.getString("image_url")).setRating(object.getDouble("rating"));
            JSONObject coordinates = (JSONObject) object.get("coordinates");
            builder.setLatitude(coordinates.getDouble("latitude")).setLongitude(coordinates.getDouble("longitude"));
            JSONObject location = (JSONObject) object.get("location");
            builder.setCity(location.getString("city")).setState(location.getString("state")).setZipcode(location.getString("zip_code"));
            JSONArray addresses = (JSONArray) location.get("display_address");
            String fullAddress = addresses.join(",");
            builder.setAddress(fullAddress).setImageUrl(object.getString("image_url")).setUrl(object.getString("url"));
